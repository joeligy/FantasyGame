			...
			if (myScript.blendShapeObjects.Count > 0)
			{
				for (int o = 0; o < myScript.blendShapeObjects.Count; o++){
					...
					if (blendShapeObject.primaryShapes > 0)
					{
						...
						myScript.blendShapeObjects[o].expandedInspector = EditorGUILayout.Foldout(myScript.blendShapeObjects[o].expandedInspector, fixedMeshName);
						...
						else
						{
							if (myScript.blendShapeObjects[o].blendShapes.Count != 0)
							{
								...
								EditorGUILayout.EndHorizontal ();
								EditorGUILayout.BeginHorizontal ();
								SFB_BlendShapesManager.SFB_BlendShape blendShapeDataReset = new SFB_BlendShapesManager.SFB_BlendShape();
								if(GUILayout.Button("Randomize All", GUILayout.Width(100)))
								{
									GUI.color 			= Color.green;
								...
								for (int i = 0; i < blendShapeObject.blendShapes.Count; i++){
									SFB_BlendShapesManager.SFB_BlendShape blendShapeData = blendShapeObject.blendShapes[i];
									if (blendShapeData.isVisible)
									{
										...
										GUILayout.Label (blendShapeData.inspectorID + ": " + displayName, GUILayout.Width(200));
										...
										if (blendShapeData.isPlus)
										{
											int minusShapeObject = myScript.GetMinusShapeObject(blendShapeData.name);
											int minusShapeID = myScript.GetMinusShapeID(blendShapeData.name);
											SFB_BlendShapesManager.SFB_BlendShape minusShapeData = myScript.blendShapeObjects[minusShapeObject].blendShapes[minusShapeID];
										...
										{
											Undo.RecordObject (myScript, "Change Blend Shape Slider Value");
											blendShapeData.sliderValue = EditorGUILayout.Slider(blendShapeData.sliderValue,0, 100);
										}



										Undo.RecordObject (myScript, "Toggle Preset Export");
										blendShapeData.presetExportValue = EditorGUILayout.Popup(blendShapeData.presetExportValue, presetExportOptions);
										...
										blendShapeData.minValue = EditorGUILayout.IntField (Mathf.RoundToInt(blendShapeData.minValue), GUILayout.Width (40));

										EditorGUILayout.MinMaxSlider( ref blendShapeData.minValue, ref blendShapeData.maxValue, minLimit, maxLimit, GUILayout.Width(150));
										blendShapeData.maxValue = EditorGUILayout.IntField(Mathf.RoundToInt(blendShapeData.maxValue), GUILayout.Width(40));
										EditorGUILayout.EndHorizontal ();
