...
public class SFB_SaveObjectAsMesh : MonoBehaviour {
	...
	static void ExportMeshes () {
		...
		for (int i = 0; i < Selection.objects.Length; i++){
			...
			string parentFolder = "Assets/SFBayStudios/Exported Meshes";							// Path to the folder that holds all of our exports
			string folderName = thisObject.name;													// Get a new folder name for this object
			int fileNumber = SFB_GetFileNumber("/SFBayStudios/Exported Meshes/" + folderName, thisObject.name);		// Get a sequential number if needed
